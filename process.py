...
def get_follow_title(df_combined):
    ...
    #热销模板
    prompt_template3 = """请对以下用","分隔的产品分类,先统计下面数组长度,然后针对每个分类生成一个营销文案，
    1)文案内容在10个中文字符以内, 带“新款”类似字眼。
    2）请使用小红书爆文风格文案，
    3）回复的内容不能有重复，开头不要带序号、分类名称或特殊符号，
    4）回复内容的行数应与之前数组统计长度相同，不要回复除营销标题以外的任何内容，
    5）不要有多余换行或空行,请务必检查，务必检查以上条件再回复我。
   ...
   #热销模板
    prompt_template4 = """请对以下用","分隔的产品分类,先统计下面数组长度,然后针对每个分类生成一个营销文案，
    1)文案内容在10个中文字符以内, 带“五一节日”祝福类似字眼，不用带推销，主要是情怀。
    2）请使用小红书爆文风格文案，
    3）回复的内容不能有重复，开头不要带序号、分类名称或特殊符号，
    4）回复内容的行数应与之前数组统计长度相同，不要回复除营销标题以外的任何内容，
    5）不要有多余换行或空行,请务必检查，务必检查以上条件再回复我。
    ...
    results = get_results_api(cal_list, prompt_template4, 10)
    print(results)
    print(len(results))
...
def get_follow_content(df_combined):
    ...
    prompt_template3 = """请对以下用","分隔的产品分类,先统计下面数组长度,然后针对每个分类生成一个营销文案，
    1)文案内容在60~80个中文字符以内,
    2）请结合知名女性作家书籍上的名言警句，创作一个营销文案，内容可以长点
    3）回复的内容不能有重复，开头不要带序号、分类名称或特殊符号，
    4）回复内容的行数应与之前数组统计长度相同，不要回复除营销标题以外的任何内容，
    5）不要有多余换行或空行,请务必检查，务必检查以上条件再回复我。
    ...
    prompt_template4 = """请对以下用","分隔的产品分类,先统计下面数组长度,然后针对每个分类生成一个营销文案，
    1)文案内容在60~80个中文字符以内,
    2）请结合关于“女性自由、独立”的著名句子，来自中外名人或文学作品，可以用于激励、宣传或品牌文案..创作一个营销文案，内容可以长点
    3）回复的内容不能有重复，开头不要带序号、分类名称或特殊符号，
    4）回复内容的行数应与之前数组统计长度相同，不要回复除营销标题以外的任何内容，
    5）不要有多余换行或空行,请务必检查，务必检查以上条件再回复我。
    ...
    #小红书穿搭热点
    prompt_template5 = """请对以下用","分隔的产品分类,先统计下面数组长度,然后针对每个分类生成一个营销文案，
    1)文案内容在60~80个中文字符以内,
    2）请结合小红书最近穿搭热点，创作一个营销文案，内容可以长点
    3）回复的内容不能有重复，开头不要带序号、分类名称或特殊符号，
    4）回复内容的行数应与之前数组统计长度相同，不要回复除营销标题以外的任何内容，
    5）不要有多余换行或空行,请务必检查，务必检查以上条件再回复我。
    ...
    prompt_template6 = """请对以下用","分隔的产品分类,先统计下面数组长度,然后针对每个分类生成一个营销文案，
    1)文案内容在60~80个中文字符以内,
    2）请使用小红书种草类文案，主要祝福大家五一节日快乐，因为客户收到信息已经是五一节期间，所以你的文案要付和这个场景。
    3）回复的内容不能有重复，开头不要带序号、分类名称或特殊符号，
    4）回复内容的行数应与之前数组统计长度相同，不要回复除营销标题以外的任何内容，
    5）不要有多余换行或空行,请务必检查，务必检查以上条件再回复我。
    6）可参考类似种草模板：【这款{{商品名称}}太适合{{适合人群/场景}}了！】 ✅ 卖点1：{{卖点1}} ✅ 卖点2：{{卖点2}} 💡 再也不用担心{{痛点场景}}了！
    7) 请用一个list的格式返回,不要带python等信息，仅返回对应分类的营销文案
    ----
    "{reviews}"
    """
    ...
    results = get_results_api(cal_list, prompt_template6, 10)
    print(results)
    print(len(results))
...
def  get_group_message():
    ...
    #因为群营销文案中文仅支持20中文字，所以要修改。
    prompt_template = """帮我写一个文案，吸引客户关注，，大概内容如右边：边缝夏4波于4.23日晚7:00直播上新，提前预览...
